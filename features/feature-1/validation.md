...
| KI-04 | `runTick` called `updateCounters` after `applyDecay`, so `fullHungerTicks` always reset immediately (hunger dropped from 100 before being checked). Fixed by moving `updateCounters` to the top of `runTick`. | `js/cat.js` → `runTick` | Fixed |