...
|---|---|---|---|
| ~~BUG-01~~ | ~~Jest/Node incompatibility: `tests/game.test.js` is browser-only and must be rewritten for Jest before CI works.~~ | ~~High~~ | ~~KI-01~~ | **Resolved** — `tests/game.test.js` rewritten; 64 tests passing. `tests/cat.test.js` added; 26 tests passing. |
...
|---|---|---|
| ~~INF-01~~ | ~~Create `package.json` and `.github/workflows/test.yml` for Jest CI.~~ | ~~High~~ | **Resolved** — `package.json`, `tests/run.js`, and `.github/workflows/test.yml` all in place. 90 tests pass on push. |
